...
export function ChartTooltip({ active, payload, label, formatValue }: ChartTooltipProps) {
  ...
  return (
    <div className="rounded-[var(--radius-sm)] border-[0.5px] border-[var(--str-default)] bg-[var(--bg-card)] px-3 py-2 shadow-lg">
      {label != null && (
        <p className="mb-1 text-[12px] text-[var(--txt-secondary)]">{String(label)}</p>
      )}
      {payload.map((p) => {
